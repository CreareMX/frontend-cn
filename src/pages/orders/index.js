...
const PersonsType = ({ apiData }) => {
  ...
  const [listaPuntoDeVenta,setlListaPuntoDeVenta] = useState([])
   ...
   const llenarLista = ()=>{
    ...
    let listaF = listaFiltrada.filter(e => e.estado == 'pedido' || e.estado == 'cancelarP')
    ...
    },
    {
      ...
      renderCell: ({ row }) => {
        ...
        return (
          ...
          skin='light'
          size='small'
          label={row.estado == 'pedido' ? 'Pendiente' : row.estado == 'cancelarP' ? 'cancelado' : ''}
          color={row.estado == 'pedido' ? 'info' : row.estado == 'cancelarP' ? 'error' : 'warning'}
  ...
  const vender = async(id) =>{
    ...
    const programar = async(id) =>{
      let lista = [...listaPuntoDeVenta]
      let objWithIdIndex = lista.findIndex((obj) => obj.id === id);
      lista[objWithIdIndex].estado = 'programado'
    ...
    const cancelar = async(id) =>{
      let lista = [...listaPuntoDeVenta]
      let objWithIdIndex = lista.findIndex((obj) => obj.id === id);
      lista[objWithIdIndex].estado = 'cancelarP'
