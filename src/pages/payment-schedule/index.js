...
import TableHeader from 'src/views/apps/payment-schedule/TableHeader'
...
const PersonsType = ({ apiData }) => {
  ...
  const [pagosLista,setPagoLista] = useState([])
   ...
   const llenarLista = ()=>{
    setPagoLista(JSON.parse(localStorage.getItem('pagosProgramados')) || []);
   }
  ...
  const pagaPago = async(id) =>{
    ...
    lista[objWithIdIndex].fechaPago = new Date().toLocaleDateString('es-MX')
    JSON.stringify(localStorage.setItem('pagosProgramados', JSON.stringify(lista) ))
    setPagoLista(lista)
...
const cancelarPago = async(id) =>{
    ...
    if(lista.length === 0){
      localStorage.removeItem('pagosProgramados')
      setPagoLista(lista)
    }else{
      JSON.stringify(localStorage.setItem('pagosProgramados', JSON.stringify(lista) ))
      setPagoLista(lista)
