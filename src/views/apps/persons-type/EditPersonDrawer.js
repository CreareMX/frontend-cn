...
import { putPersonType } from 'src/api/RequestApi'
...
const SidebarEditPeople = props => {
  ...
  const [dataEdit, setDataEdit]  = useState({ nombre: "" });
  ...
  useEffect(()=>{
    ...
    setEsPersonaMoral(editPerson.esPersonaMoral)
    reset(editPerson)
    },[reset, editPerson])
  ...
  const onSubmit = async(data) => {
    try {
      ...
      const response = await putPersonType(data, 1)
