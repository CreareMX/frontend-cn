...
import { postPeople } from 'src/api/RequestApi'
...
const showErrors = (field, valueLen, min) => {
  if (valueLen === 0) {
    ...
    return `${field} debe ser mayor a ${min} caracteres`
  } else {
    return ''
  }
}
...
const schema = yup.object().shape({
  nombre: yup
    ...
    idTipoPersona: yup
    .string()
    .min(1, obj => showErrors('El tipo de persona', obj.value.length, obj.min))
    .required()
})
...
const SidebarAddUser = props => {
  // ** Props
  const { open, toggle, sucess, typePersons } = props
  ...
  const onSubmit = async(data) => {
    try {
      ...
      const response = await postPeople(data, 1)
  ...
  return (
    ...
    >
      <Header>
        <Typography variant='h6'>Agregar una persona</Typography>
        <IconButton
          size='small'
          onClick={handleClose}
